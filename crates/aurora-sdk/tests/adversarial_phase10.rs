...
fn adversarial_sdk_client_builder_and_extensions() {
    ...
    assert!(processed.params.get("correlation_id").is_some(), "correlation_id must be added");
    assert!(processed.params.get("sdk_timestamp").is_some(), "sdk_timestamp must be added");
}
