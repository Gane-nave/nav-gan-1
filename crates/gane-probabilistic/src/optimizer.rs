...
use gane_confidence::calculator::SegmentEstimate;
use gane_confidence::ConfidenceCalculator;
use gane_core::map::RoadSegment;
use gane_core::route::EtaDistribution;
use gane_core::types::EntityId;
use gane_risk::engine::{EnvironmentConditions, RiskEngine, SegmentHistory};
use tracing::debug;
...
fn estimate_energy(total_distance: f64, segments: &[RoadSegment]) -> f64 {
    ...
    let unpaved_fraction = segments
        .iter()
        .filter(|s| {
            s.surface_type == gane_core::map::SurfaceType::Gravel
                || s.surface_type == gane_core::map::SurfaceType::Dirt
        })
        .count() as f64
