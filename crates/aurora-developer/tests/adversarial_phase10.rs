...
fn adversarial_rotate_key_at_max_capacity() {
    ...
    let (key1, secret1) = mgr.create_key("key1", owner, vec![Permission::PositionRead], ApiTier::Developer, None).unwrap();
    let (_key2, secret2) = mgr.create_key("key2", owner, vec![Permission::RouteRead], ApiTier::Developer, None).unwrap();
...
fn adversarial_webhook_health_total_failure_count() {
    ...
    let wh = mgr.register(owner, "test-hook", "https://example.com/hook", "secret", vec![WebhookEvent::PositionUpdate]).unwrap();
    ...
    for i in 0..5u32 {
        ...
        if i < 4 {
            assert_eq!(status, DeliveryStatus::Retrying, "attempt {} should retry", i+1);
        } else {
            assert_eq!(status, DeliveryStatus::Failed, "attempt 5 should fail permanently");
        }
    }

    let wh_after_fail = mgr.get(&wh.id).unwrap();
    assert_eq!(wh_after_fail.total_failure_count, 1, "total_failure_count must be 1");
    assert_eq!(wh_after_fail.failure_count, 1, "consecutive failure_count must be 1");
    ...
    assert_eq!(wh_after_success.failure_count, 0, "consecutive failure_count must be reset to 0");
    assert_eq!(wh_after_success.total_failure_count, 1, "total_failure_count must still be 1");

    // CRITICAL: health() must use total_failure_count
    let health = mgr.health(&wh.id).unwrap();
    assert_eq!(health.total_deliveries, 2, "total must be 2 (1 fail + 1 success)");
    assert!((health.success_rate - 0.5).abs() < 0.001,
        "success_rate must be 0.5, got {}", health.success_rate);
}
...
fn adversarial_rate_limiting_token_bucket() {
    ...
    assert!(limiter.try_acquire(&key).is_ok(), "request 1 must be allowed");
    assert!(limiter.try_acquire(&key).is_ok(), "request 2 must be allowed");
    assert!(limiter.try_acquire(&key).is_ok(), "request 3 must be allowed");
    assert!(limiter.try_acquire(&key).is_err(), "request 4 must be denied (bucket empty)");
}
