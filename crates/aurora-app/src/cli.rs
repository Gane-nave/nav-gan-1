...
mod tests {
    ...
    #[test]
    fn default_args() {
        let cli = CliArgs::from_args(&args("aurora-nav"));
    ...
    fn config_path_parsed() {
        let cli = CliArgs::from_args(&args("aurora-nav --config /etc/aurora.toml"));
        assert_eq!(cli.config_path.unwrap(), PathBuf::from("/etc/aurora.toml"));
    }

    #[test]
    fn short_config_flag() {
        let cli = CliArgs::from_args(&args("aurora-nav -c aurora.toml"));
        assert_eq!(cli.config_path.unwrap(), PathBuf::from("aurora.toml"));
    }

    #[test]
    fn port_override() {
        let cli = CliArgs::from_args(&args("aurora-nav --port 9090"));
        assert_eq!(cli.port, Some(9090));
    }

    #[test]
    fn log_level_override() {
        let cli = CliArgs::from_args(&args("aurora-nav --log-level debug"));
        assert_eq!(cli.log_level.unwrap(), "debug");
    }

    #[test]
    fn dump_config_flag() {
        let cli = CliArgs::from_args(&args("aurora-nav --dump-config"));
        assert!(cli.dump_config);
    }

    #[test]
    fn version_flag() {
        let cli = CliArgs::from_args(&args("aurora-nav --version"));
        assert!(cli.version);
    }
    ...
    fn combined_flags() {
        let cli = CliArgs::from_args(&args(
            "aurora-nav -c config.toml --port 8080 -l warn --status",
        ));
