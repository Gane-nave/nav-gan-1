//! Map update pipeline with versioning and rollback.

use serde::{Deserialize, Serialize};

/// Entry in the MapUpdatePipeline.
...
/// Map update pipeline with versioning and rollback.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MapUpdatePipeline {
    entries: Vec<Entry>,
    updates: u64,
    max_entries: usize,
}

impl MapUpdatePipeline {
...
impl Default for MapUpdatePipeline {
    fn default() -> Self {
        Self::new(10_000)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_new() {
        let s = MapUpdatePipeline::new(100);
        assert_eq!(s.count(), 0);
    }

    #[test]
    fn test_default() {
        let s = MapUpdatePipeline::default();
        assert_eq!(s.updates(), 0);
    }

    #[test]
    fn test_add() {
        let mut s = MapUpdatePipeline::new(100);
    ...
    #[test]
    fn test_capacity() {
        let mut s = MapUpdatePipeline::new(2);
        assert!(s.add(make_entry(1)));
