...
use aurora_core::scoring::{RiskComponent, RiskEntityType, RiskFactor, RiskScore};
...
fn infra_risk(seg: &RoadSegment) -> f64 {
    ...
    if seg.surface_type == aurora_core::map::SurfaceType::Gravel
        || seg.surface_type == aurora_core::map::SurfaceType::Dirt
    {
        risk += 0.3;
    }
    risk.min(1.0)
