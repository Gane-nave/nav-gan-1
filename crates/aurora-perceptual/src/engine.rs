...
pub struct StabilityAnalyzer {
    ...
    pub max_jitter_threshold: f64,
}
impl StabilityAnalyzer {
    pub fn new(threshold: f64) -> Self {
        Self {
            samples: Vec::new(),
            max_jitter_threshold: threshold,
        }
    }
    pub fn add_sample(&mut self, s: PositionSample) {
        self.samples.push(s);
    }
    ...
    pub fn is_stable(&self) -> bool {
        self.jitter() <= self.max_jitter_threshold
    }
    ...
    pub fn stability_score(&self) -> f64 {
        (1.0 - (self.jitter() / self.max_jitter_threshold).min(1.0)).clamp(0.0, 1.0)
    }
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_stable() {
        let mut a = StabilityAnalyzer::new(1.0);
        for i in 0..5 {
            a.add_sample(PositionSample {
                x: i as f64,
                y: 0.0,
    ...
    #[test]
    fn test_jittery() {
        let mut a = StabilityAnalyzer::new(0.1);
        a.add_sample(PositionSample {
            x: 0.0,
            y: 0.0,
            timestamp_ms: 0,
    ...
    fn test_smooth() {
        ...
        let (x, y) = a.smooth_position().unwrap();
        assert!((x - 2.0).abs() < 0.01);
        assert!((y - 3.0).abs() < 0.01);
    }
    #[test]
    fn test_empty() {
        let a = StabilityAnalyzer::new(1.0);
        assert!(a.smooth_position().is_none());
