//! Data integrity verification for maps and traffic data.

use serde::{Deserialize, Serialize};

/// Entry in the MapSanityChecker.
...
/// Data integrity verification for maps and traffic data.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MapSanityChecker {
    entries: Vec<Entry>,
    anomalies: u64,
    max_entries: usize,
}

impl MapSanityChecker {
...
impl Default for MapSanityChecker {
    fn default() -> Self {
        Self::new(10_000)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_new() {
        let s = MapSanityChecker::new(100);
        assert_eq!(s.count(), 0);
    }

    #[test]
    fn test_default() {
        let s = MapSanityChecker::default();
        assert_eq!(s.anomalies(), 0);
    }

    #[test]
    fn test_add() {
        let mut s = MapSanityChecker::new(100);
    ...
    #[test]
    fn test_capacity() {
        let mut s = MapSanityChecker::new(2);
        assert!(s.add(make_entry(1)));
