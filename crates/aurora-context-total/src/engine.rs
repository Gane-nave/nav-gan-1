...
impl TotalContext {
    pub fn complexity_score(&self) -> f64 {
        ...
        let state = match self.driving_state {
            DrivingState::Highway => 0.3,
            DrivingState::Urban => 0.6,
            DrivingState::Tunnel => 0.7,
            DrivingState::Parking => 0.5,
            _ => 0.2,
        } * 0.1;
        (speed + weather + traffic + cognitive + fatigue + state).clamp(0.0, 1.0)
    }
    pub fn requires_simplified_ui(&self) -> bool {
        self.complexity_score() > 0.6
    }
    pub fn is_night(&self) -> bool {
        self.hour < 6.0 || self.hour > 20.0
    }
    pub fn recommended_info_density(&self) -> f64 {
        (1.0 - self.complexity_score()).clamp(0.2, 1.0)
    }
}
#[cfg(test)]
mod tests {
    use super::*;
    fn base() -> TotalContext {
        TotalContext {
            lat: 32.0,
            lon: 34.0,
            speed_mps: 15.0,
            ...
            hour: 14.0,
            day_of_week: 2,
            driving_state: DrivingState::Urban,
    ...
    #[test]
    fn test_normal() {
        assert!(!base().requires_simplified_ui());
    }
    #[test]
    fn test_complex() {
        let mut c = base();
        c.weather = WeatherCondition::Storm;
        c.traffic_congestion = 0.9;
        c.cognitive_load = 0.8;
        assert!(c.requires_simplified_ui());
    }
    #[test]
    fn test_night() {
        let mut c = base();
    ...
    #[test]
    fn test_density() {
        assert!(base().recommended_info_density() > 0.0);
    }
}
