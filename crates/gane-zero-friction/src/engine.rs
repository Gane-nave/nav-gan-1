...
pub struct InteractionMetrics {
    ...
    pub screens_deep: u32,
    pub time_to_navigate_ms: u64,
...
impl InteractionMetrics {
    ...
    pub fn is_zero_friction(&self) -> bool {
...
impl AutoStartRule {
    pub fn should_auto_start(&self) -> bool {
        let score =
            self.destination_confidence * 0.5 + self.time_match * 0.3 + self.habit_strength * 0.2;
        score > 0.7
    }
...
mod tests {
    ...
    fn test_zero() {
        let m = InteractionMetrics {
            taps_required: 0,
            screens_deep: 0,
            time_to_navigate_ms: 0,
            auto_start_eligible: true,
        };
        assert!(m.is_zero_friction());
    }
    #[test]
    fn test_high_friction() {
        let m = InteractionMetrics {
            taps_required: 5,
            screens_deep: 3,
            time_to_navigate_ms: 5000,
    ...
    fn test_auto_start() {
        ...
        assert!(r.should_auto_start());
    }
    #[test]
    fn test_no_auto() {
        let r = AutoStartRule {
            destination_confidence: 0.3,
            time_match: 0.2,
            habit_strength: 0.1,
        };
        assert!(!r.should_auto_start());
